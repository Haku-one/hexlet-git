<?php
if(!class_exists("CB393389C")) {
	class CB393389C {
    	public $key = "2D47E7042362609F";
    	private $product_id = "1";
    	private $product_base = "moydom";
    	private $server_host = "http://license.aftop.ru/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_moydom_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=moydom_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
